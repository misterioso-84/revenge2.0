...
export function UserProfileDropdown({ className = "" }: { className?: string }) {
  ...
  const roleLabel = isAdmin
    ? "Amministratore"
    : customRoleNames[0] || (hasEmployeeAccess ? "Staff" : "Cittadino");
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className={`flex items-center gap-2.5 bg-slate-900/90 hover:bg-slate-800/90 border border-amber-500/30 hover:border-amber-400/60 rounded-2xl px-2.5 py-1.5 shadow-lg transition-all duration-200 outline-none focus-visible:ring-2 focus-visible:ring-amber-400 group cursor-pointer ${className}`}
        >
          <div className="relative shrink-0">
            <img
              src={avatarUrl}
              alt={username}
              className="h-8 w-8 rounded-xl border border-amber-500/50 group-hover:border-amber-400 object-cover shadow-sm transition-transform duration-200 group-hover:scale-105"
              onError={(e) => {
          ...
          <ChevronDown className="h-3.5 w-3.5 text-slate-400 group-hover:text-amber-400 transition-transform duration-200 group-data-[state=open]:rotate-180" />
        </button>
      </DropdownMenuTrigger>
      ...
      >
        ...
        {/* STAFF / ADMIN PANEL LINK (Se autorizzato) */}
        {hasEmployeeAccess && (
          <DropdownMenuItem
            asChild
            ...
            <Link to="/dashboard" className="flex items-center gap-2.5 w-full">
              ...
              <div className="flex flex-col text-left flex-1">
                <span className="font-extrabold text-white text-xs">
                  {isAdmin ? "Pannello Amministratore" : "Pannello Gestionale Staff"}
                </span>
